...
class TestOptimizationStrategy:
    ...
    def test_all_members(self) -> None:
        assert len(OptimizationStrategy) == 4
        assert OptimizationStrategy.STANDARD is not None
        assert OptimizationStrategy.STREAMING is not None


class TestIsMemoryError:
    """Tests for _is_memory_error()."""

    def test_memory_error_type(self) -> None:
        assert _is_memory_error(MemoryError("oom"))

    def test_oom_in_message(self) -> None:
        assert _is_memory_error(RuntimeError("XLA OOM"))

    def test_allocate_keyword(self) -> None:
        assert _is_memory_error(RuntimeError("failed to allocate buffer"))

    def test_not_memory_error(self) -> None:
        assert not _is_memory_error(ValueError("bad value"))


class TestGetFallbackStrategy:
    """Tests for get_fallback_strategy()."""

    def test_standard_to_large(self) -> None:
        result = get_fallback_strategy(OptimizationStrategy.STANDARD)
        assert result == OptimizationStrategy.LARGE

    def test_large_to_chunked(self) -> None:
        result = get_fallback_strategy(OptimizationStrategy.LARGE)
        assert result == OptimizationStrategy.CHUNKED

    def test_chunked_to_streaming(self) -> None:
        result = get_fallback_strategy(OptimizationStrategy.CHUNKED)
        assert result == OptimizationStrategy.STREAMING

    def test_streaming_returns_none(self) -> None:
        result = get_fallback_strategy(OptimizationStrategy.STREAMING)
        assert result is None

    def test_memory_error_skips_to_chunked(self) -> None:
        err = MemoryError("oom")
        result = get_fallback_strategy(OptimizationStrategy.STANDARD, error=err)
        assert result == OptimizationStrategy.CHUNKED

    def test_memory_error_from_large_goes_to_chunked(self) -> None:
        err = MemoryError("oom")
        result = get_fallback_strategy(OptimizationStrategy.LARGE, error=err)
        assert result == OptimizationStrategy.CHUNKED

    def test_memory_error_from_chunked_goes_to_streaming(self) -> None:
        err = MemoryError("oom")
        result = get_fallback_strategy(OptimizationStrategy.CHUNKED, error=err)
        assert result == OptimizationStrategy.STREAMING

    def test_memory_error_from_streaming_returns_none(self) -> None:
        err = MemoryError("oom")
        result = get_fallback_strategy(OptimizationStrategy.STREAMING, error=err)
        assert result is None

    def test_non_memory_error_follows_order(self) -> None:
...
class TestCreateStrategy:
    """Tests for _create_strategy()."""

    def test_creates_standard(self) -> None:
        from heterodyne.optimization.nlsq.fallback_chain import _create_strategy
        from heterodyne.optimization.nlsq.strategies.residual import ResidualStrategy

        strategy = _create_strategy(OptimizationStrategy.STANDARD)
        assert isinstance(strategy, ResidualStrategy)

    def test_creates_large(self) -> None:
        from heterodyne.optimization.nlsq.fallback_chain import _create_strategy
        from heterodyne.optimization.nlsq.strategies.jit_strategy import JITStrategy

        strategy = _create_strategy(OptimizationStrategy.LARGE)
        assert isinstance(strategy, JITStrategy)

    def test_creates_chunked(self) -> None:
        from heterodyne.optimization.nlsq.fallback_chain import _create_strategy
        from heterodyne.optimization.nlsq.strategies.chunked import ChunkedStrategy

        strategy = _create_strategy(OptimizationStrategy.CHUNKED)
        assert isinstance(strategy, ChunkedStrategy)

    def test_creates_streaming(self) -> None:
        from heterodyne.optimization.nlsq.fallback_chain import _create_strategy
        from heterodyne.optimization.nlsq.strategies.sequential import SequentialStrategy

        strategy = _create_strategy(OptimizationStrategy.STREAMING)
        assert isinstance(strategy, SequentialStrategy)


class TestExecuteOptimizationWithFallback:
    """Tests for execute_optimization_with_fallback() via mocked strategies."""

    def test_auto_select_standard_for_small_data(self) -> None:
        """Verify auto-selection picks STANDARD for small data."""
        ...
        mock_result = SimpleNamespace(
            result=SimpleNamespace(success=True, final_cost=0.1, message="ok"),
            metadata={},
        )
        mock_strategy = MagicMock()
        mock_strategy.fit.return_value = mock_result
        mock_strategy.name = "MockStrategy"

        with patch(
            "heterodyne.optimization.nlsq.fallback_chain._create_strategy",
            return_value=mock_strategy,
        ):
            result = execute_optimization_with_fallback(
                model=MagicMock(),
                c2_data=np.ones((50, 50)),  # 2500 < 10000 => STANDARD
                phi_angle=0.0,
                config=MagicMock(),
            )
    ...
    def test_auto_select_large_for_medium_data(self) -> None:
        """Verify auto-selection picks LARGE for medium data."""
        from unittest.mock import patch
        ...
        with patch(
            ...
            result = execute_optimization_with_fallback(
                model=MagicMock(),
                c2_data=np.ones((200, 200)),  # 40000 => LARGE
                phi_angle=0.0,
                config=MagicMock(),
            )
            assert result.result.success is True

    def test_auto_select_chunked_for_large_data(self) -> None:
        """Verify auto-selection picks CHUNKED for large data."""
        from unittest.mock import patch
        ...
        with patch(
            ...
            result = execute_optimization_with_fallback(
                model=MagicMock(),
                c2_data=np.ones((500, 500)),  # 250000 => CHUNKED
                phi_angle=0.0,
                config=MagicMock(),
            )
            assert result.result.success is True

    def test_fallback_on_non_convergence(self) -> None:
        """Verify fallback chain progresses when strategy doesn't converge."""
        from unittest.mock import patch
        ...
        fail_result = SimpleNamespace(
            result=SimpleNamespace(success=False, final_cost=99.0, message="no conv"),
            metadata={},
        )
        ...
        def mock_create(strategy):
            mock = MagicMock()
            mock.name = f"Mock-{strategy.name}"
            call_count["n"] += 1
            if call_count["n"] == 1:
                mock.fit.return_value = fail_result
            else:
                mock.fit.return_value = ok_result
            return mock
    ...
    def test_fallback_on_exception(self) -> None:
        """Verify fallback chain progresses on exception."""
        from unittest.mock import patch
        ...
        def mock_create(strategy):
            ...
            call_count["n"] += 1
            if call_count["n"] == 1:
                mock.fit.side_effect = ValueError("boom")
            else:
                mock.fit.return_value = ok_result
            return mock
    ...
    def test_all_strategies_fail_raises(self) -> None:
        """Verify RuntimeError when all strategies fail."""
        from unittest.mock import patch
        ...
        def mock_create(strategy):
            mock = MagicMock()
            mock.name = f"Mock-{strategy.name}"
            mock.fit.side_effect = ValueError("always fails")
            return mock
        ...
        with (
            ...
            pytest.raises(RuntimeError, match="All optimization strategies failed"),
        ):
            execute_optimization_with_fallback(
                model=MagicMock(),
                c2_data=np.ones((50, 50)),
