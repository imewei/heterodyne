...
class NLSQAdapterBase(ABC):
    ...
    def _validate_input(
        ...
        logger.debug("Input validation passed: %d points", n)
    ...
    def _setup_bounds(
        ...
        logger.debug("Bounds setup: %d parameters", n_params)
        return lower, upper
    ...
    def _compute_covariance(
        ...
        try:
            ...
            if dof <= 0:
                logger.warning(
                    "Insufficient degrees of freedom: %d points, %d params",
                    n_points,
                    n_params,
                )
                return None
            ...
            if cond > 1e12:
                logger.warning("J^T J ill-conditioned (cond=%.2e), using SVD", cond)
            ...
            logger.warning("Covariance computation failed: %s", e)
            return None

    # ---------------------------------------------------------------------------
    # Heterodyne-specific typed fit overload (kept for JAX residual-fn path)
    # The abstract fit(self, *args, **kwargs) above is satisfied by concrete
    # subclass implementations — NLSQAdapter and NLSQWrapper both override fit().
    # ---------------------------------------------------------------------------
    ...
    def fit_typed(
        ...
        """Run optimization with typed JAX residual-function signature.

        Convenience alias for subclasses that implement the heterodyne
        low-level residual-function interface. Delegates to fit().
