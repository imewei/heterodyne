...
def generate_plots(
    ...
    if nlsq_results:
        for result in nlsq_results:
            phi = result.metadata.get("phi_angle", 0)
    ...
    # CMC plots
    if cmc_results:
        for result in cmc_results:
            phi = result.metadata.get("phi_angle", 0)
            suffix = f"_phi{int(phi)}" if len(cmc_results) > 1 else ""
            
            plot_posterior(result, save_path=plots_dir / f"cmc_posterior{suffix}.png")
            plot_trace(result, save_path=plots_dir / f"cmc_trace{suffix}.png")
