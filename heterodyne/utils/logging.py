...
class MinimalLogger:
    ...
    def configure(
        ...
        *,
        ...
        Args:
            ...
            console_level: Console handler level. Pass ``False`` to suppress
                the console handler entirely.
            console_format: "simple" or "detailed".
            console_colors: Enable ANSI color output on the console.
            file_path: Path to log file. ``None`` disables file logging.
            file_level: File handler level. Pass ``False`` to suppress the
                file handler even when *file_path* is set.
            max_size_mb: Rotating log file maximum size in MB.
            backup_count: Number of backup files to keep.
            module_levels: Per-module level overrides.
            force: Remove existing managed handlers before reconfiguring.
    ...
    def configure_from_dict(
        ...
        *,
        ...
        Args:
            logging_config: Mapping with logging configuration (may be ``None``
                or contain ``enabled: false`` to skip configuration).
            verbose: Override console level to DEBUG.
            quiet: Override console level to ERROR.
            output_dir: Base directory for auto-generated log file paths.
            run_id: Run identifier used for auto-generated log filenames.
...
def configure_logging(
    ...
    Simple interface for configuring console (and optionally file) logging.
    For richer control — rotating files, per-module overrides, CLI flag
    integration — use :class:`LogConfiguration` instead.

    Args:
        level: Logging level.
        log_file: Optional path to log file.
        format_string: Custom format string, or ``None`` for the default
            detailed format.
    """
    ...
    # Mark configured so MinimalLogger does not auto-reconfigure
    _logger_manager._configured = True
...
def log_performance(
    ...
    Only emits a log record when the function's wall-clock time meets or
    ...
    Args:
        logger: Logger to use. If ``None``, creates one for the decorated
            function's module.
        level: Logging level to use.
        threshold: Minimum duration in seconds required to emit a log record.
