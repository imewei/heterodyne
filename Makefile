# Heterodyne Analysis Package Makefile
# =====================================
...
# ===================
help:
	@echo "$(BOLD)$(BLUE)Heterodyne Analysis Package - Development Commands$(RESET)"
	@echo ""
	...
	@echo "  $(CYAN)test-scientific$(RESET)        Run scientific tests only"
	@echo "  $(CYAN)test-nlsq$(RESET)              Test NLSQ optimization"
	@echo "  $(CYAN)test-cmc$(RESET)               Test CMC optimization"
	...
	@echo "  $(CYAN)profile-cmc$(RESET)      Profile CMC optimization"
	@echo ""
...
# ===================
env-info:
	@echo "$(BOLD)$(BLUE)Heterodyne Environment Information$(RESET)"
	@echo "==================================="
	@echo ""
...
deps-check:
	@echo "$(BOLD)$(BLUE)Checking Heterodyne Dependencies...$(RESET)"
	@printf "  JAX: " && $(PYTHON) -c "import jax; print('✓', jax.__version__)" 2>/dev/null || echo "✗ not installed"
	@printf "  NumPy: " && $(PYTHON) -c "import numpy; print('✓', numpy.__version__)" 2>/dev/null || echo "✗ not installed"
	@printf "  SciPy: " && $(PYTHON) -c "import scipy; print('✓', scipy.__version__)" 2>/dev/null || echo "✗ not installed"
	@printf "  NLSQ: " && $(PYTHON) -c "import nlsq; print('✓', nlsq.__version__)" 2>/dev/null || echo "✗ not installed"
	@printf "  NumPyro: " && $(PYTHON) -c "import numpyro; print('✓', numpyro.__version__)" 2>/dev/null || echo "✗ not installed"
...
test-smoke:
	...
	$(RUN_CMD) $(PYTEST) $(TEST_DIR) -v --tb=short -x -q -m "not slow"
...
test-fast:
	...
	$(RUN_CMD) $(PYTEST) $(TEST_DIR) -m "not slow" -v --tb=short
...
test-unit:
	...
	$(RUN_CMD) $(PYTEST) $(TEST_DIR) -v --tb=short -m "unit"
...
lint-tests:
	@echo "$(BOLD)$(BLUE)Running linting checks on tests...$(RESET)"
	$(RUN_CMD) $(RUFF) check $(TEST_DIR)
	@echo "$(BOLD)$(GREEN)✓ No linting errors in tests!$(RESET)"
...
benchmark:
	...
	$(RUN_CMD) $(PYTEST) $(TEST_DIR) -v -m performance --tb=short 2>/dev/null || \
	echo "No performance tests found. Add @pytest.mark.performance to benchmark tests."

profile-nlsq:
	@echo "$(BOLD)$(BLUE)Profiling NLSQ optimization...$(RESET)"
	@$(PYTHON) -c "from heterodyne.optimization.nlsq import fit_nlsq_jax; print('✓ NLSQ profiling ready')"

profile-cmc:
	@echo "$(BOLD)$(BLUE)Profiling CMC optimization...$(RESET)"
	@$(PYTHON) -c "from heterodyne.optimization.cmc import fit_cmc_jax; print('✓ CMC profiling ready')"
...
clean-test:
	...
	rm -rf heterodyne_results/ .heterodyne_cache/ cmc_temp_*/ tmp/ .ultra-think/
	rm -f bandit*report.json phi_angles_list.txt TASK_GROUP_*.md heterodyne_*.log
...
clean: clean-build clean-pyc clean-test
	...
	@echo "$(BOLD)Protected directories preserved:$(RESET) .venv/, venv/, .claude/, .specify/, agent-os/, .serena/"
...
# ===================
stats:
	@echo "$(BOLD)$(BLUE)Heterodyne Project Statistics$(RESET)"
	@echo "=============================="
	@echo "Lines of code:"
	...
	@echo "Test count: $(shell $(RUN_CMD) $(PYTEST) $(TEST_DIR) --collect-only -q 2>/dev/null | tail -n 1)"
...
verify-nlsq:
	...
	@$(PYTHON) -c "from heterodyne.optimization.nlsq import NLSQAdapter; print('✓ NLSQAdapter imported')"
	@$(PYTHON) -c "from heterodyne.optimization.nlsq import fit_nlsq_jax; print('✓ fit_nlsq_jax imported')"
...
# ===================
verify-cmc:
	@echo "$(BOLD)$(BLUE)Verifying CMC integration...$(RESET)"
	@$(PYTHON) -c "from heterodyne.optimization.cmc import fit_cmc_jax; print('✓ fit_cmc_jax imported')"
	@$(PYTHON) -c "import numpyro; print(f'✓ NumPyro version: {numpyro.__version__}')" 2>/dev/null || echo "✗ NumPyro version check failed"
	@echo "$(BOLD)$(GREEN)✓ CMC integration verified!$(RESET)"
