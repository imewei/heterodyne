...
- Heterodyne SHA: `3ca229f56535c0a2e19120ce39bdf68fb73d1b8c`
- Total gaps: **4503**
...
| `KEEP` | missing_class | `optimization.nlsq.wrapper.NLSQWrapper` | homodyne defines this class; heterodyne does not |
...
| `KEEP` | missing_in_heterodyne | `optimization.nlsq.wrapper.NLSQWrapper.fit` | homodyne has `fit(self, data: Any, config: Any, initial_params: np.ndarray | None = None, bounds: tuple[np.ndarray, np.ndarray] | None = None, analysis_mode: str = 'static_isotropic', per_angle_scaling: bool = True, diagnostics_enabled: bool = False, shear_transforms: dict[str, Any] | None = None, per_angle_scaling_initial: dict[str, list[float]] | None = None) -> OptimizationResult`; heterodyne missing |
...
## P1 — Structural drift — files added/missing/renamed, docs pages missing (1165 gaps)
...
| `KEEP` | missing_py_file | `optimization.nlsq.wrapper` | homodyne has `optimization.nlsq.wrapper.py`; heterodyne does not |
...
Both implementations expose a `get_fallback_strategy()` function and an `execute_optimization_with_fallback()` entry point, but the chain topology differs in one load-bearing way: homodyne has a 4-step chain (`STREAMING → CHUNKED → LARGE → STANDARD`) encoded as a static dict in `get_fallback_strategy()`, whereas heterodyne has a 3-step chain (`STREAMING → LARGE → STANDARD`, defined as `_FALLBACK_ORDER` at line 55) that omits the CHUNKED level because the heterodyne model passes a single `(N, N)` two-time matrix rather than the angle-stratified chunk structure that CHUNKED requires. The heterodyne version also adds a **memory-error fast-path** inside `get_fallback_strategy()`: when the error is OOM-class and the failing strategy is LARGE, it skips directly to STREAMING rather than stepping to STANDARD, which would not reduce peak memory. Both versions catch `MemoryError`, `ValueError`, and `RuntimeError` at the same level; the heterodyne version additionally calls `_is_memory_error()` to detect OOM-flavoured `RuntimeError` strings ("oom", "out of memory", "allocate", "mmap"). The abort condition is identical — `RuntimeError` is raised when the chain is exhausted. The heterodyne refactor also decoupled result normalisation into `result_builder.build_result_from_nlsq()` rather than the inline `handle_nlsq_result()` present in homodyne, which is an internal restructuring with no semantic impact. **Assessment:** the CHUNKED omission is intentional and physics-exempt; the OOM fast-path is a load-bearing heterodyne addition that Phase 4 should replicate back into homodyne rather than remove.

### cmc/sampler.py

Both versions share the same `SamplingPlan` + `NUTSSampler` object model and the same `dense_mass=True` default, but diverge significantly in retry semantics and the warmup floor contract. Homodyne exposes a separate top-level function `run_nuts_with_retry()` (line 1205) that wraps `run_nuts_sampling()` with up to `max_retries` (default 3) attempts: on each failed attempt it escalates `target_accept_prob` by +0.05 (capped at 0.95) and uses a fresh `jax.random.fold_in` key, applying a tighter divergence threshold (`DIVERGENCE_RATE_TARGET`) on attempt 1 and a looser one (`DIVERGENCE_RATE_HIGH`) on subsequent attempts. Homodyne also performs a `_preflight_log_density()` before sampling and applies an automatic `target_accept_prob` elevation to 0.9 for `laminar_flow` mode (line 822). Heterodyne has **no `run_nuts_with_retry` function** — `NUTSSampler.run()` makes a single attempt with no divergence-triggered retry loop; divergences are detected and logged by `get_divergence_stats()` but control is returned to the caller, leaving re-run decisions to the CMC core. Heterodyne does enforce Rule 12 (NUTS warmup floor) explicitly through `effective_warmup_floor()` called at two sites: `SamplingPlan.from_config()` (line 287) and `SamplingPlan.for_shard()` (line 341), with a `fast_warmup` escape hatch for CI. Homodyne's warmup floor is enforced only inside `SamplingPlan.from_config()` via `get_adaptive_sample_counts()`. **Assessment:** the missing `run_nuts_with_retry` is a real parity gap. Phase 4 should port the escalation loop (including the `laminar_flow` `target_accept_prob` elevation, translated to heterodyne's `two_component` analysis mode). The warmup floor contract is more robust in heterodyne and should be back-ported to homodyne.

### cli/commands.py

The most significant structural difference is the **return type**: homodyne's `dispatch_command()` returns `dict[str, Any]` (with `{"success": True/False, ...}`) while heterodyne's returns `int` (0 on success, 2 on convergence failure). Heterodyne's exit-code 2 path (line 555) was added deliberately to let CI detect the het_a10cf27e-class regression even when the human reader misses the "not_converged" summary line; homodyne relies on the caller inspecting the returned dict's `"success"` key. Both dispatchers use the same phase-logging pattern (`log_phase` + `AnalysisSummaryLogger`) and both catch `KeyboardInterrupt` and broad `Exception` at the top level. Homodyne checks `HAS_CORE_MODULES` at entry and returns early with `{"success": False}` if imports failed; heterodyne assumes imports succeed and lets `ImportError` propagate to the top-level `except Exception` handler. The plot-only short-circuits in heterodyne (lines 344–438) return integer exit codes immediately rather than building a full result dict, giving cleaner shell-script integration. **Assessment:** the `int` return type is a deliberate heterodyne improvement that Phase 4 should adopt in homodyne; the import-guard difference is a minor softness in heterodyne that is acceptable given the installed-package assumption.

### anti_degeneracy_controller.py

Homodyne implements a **5-layer** `AntiDegeneracyController` orchestrated by `_initialize_components()` (Layers 1–5: Fourier reparameterization, hierarchical optimization, adaptive regularization, gradient collapse monitoring, shear-sensitivity weighting), whereas heterodyne implements a **single-class `DegeneracyChecker`** that runs three diagnostic checks in sequence — correlation degeneracy (`_check_correlation()`), bound saturation (`_check_bounds()`), and cost plateau (`_check_plateau()`) — via its `check()` method (line 96). The heterodyne class is diagnostic-only: it returns a `DegeneracyCheck` dataclass summarising the worst issue but takes no corrective action itself (no Fourier transform, no regularizer, no shear weighting). Homodyne's Layer 4 (`GradientCollapseMonitor`) and Layer 5 (`ShearSensitivityWeighting`) have no heterodyne counterparts, and this is correct per spec: Layer 5 relies on a homodyne-specific sinc shear term that does not apply to the heterodyne velocity-phase model. Heterodyne also exposes a module-level function `detect_hierarchical_trigger()` (line 407) that decides whether to invoke hierarchical fitting based on the `DegeneracyChecker` output, keeping the trigger logic decoupled from the diagnostic logic. **Assessment:** the architecture difference is intentional and physics-exempt for Layer 5. Layers 1–4 proactive corrections exist in heterodyne only as lighter diagnostic signals; if homodyne's Phase 4 alignment requires porting the full `AntiDegeneracyController` structure, it would need the heterodyne residual architecture to support stratified chunks first.

### recovery.py

Both modules implement `execute_with_recovery()` with the same 3-recovery-action sequence — perturbation → tolerance relaxation → method switching — but the function signatures and convergence criteria differ. Homodyne's version (line 40) takes raw NLSQ primitives (`residual_fn`, `xdata`, `ydata`, `curve_fit_fn`, `curve_fit_large_fn`) and loops `max_retries` (hardcoded 3) times checking convergence internally via cost reduction heuristics and `FunctionEvaluationCounter`; it also detects **parameter stagnation** as an additional mid-attempt trigger (line 214) and injects 10% perturbation proactively. Heterodyne's version (line 160) takes a higher-level `fit_fn: Callable[[params, bounds, config], NLSQResult]` and iterates `max_retries + 1` (4 total) attempts — one initial plus 3 recoveries — checking success via `result.success` rather than cost deltas. Heterodyne introduces `diagnose_error()` (line 54) which classifies the exception into categories (`"memory"`, `"convergence"`, `"bounds"`, `"numerical"`, `"tolerance"`) and sets `recoverable=False` for unknown errors, causing an early `break` rather than exhausting all retries. The heterodyne version uses `copy.copy(config)` for the tolerance relaxation so the caller's config is never mutated, a safety property homodyne's in-place kwarg approach lacks. **Assessment:** no phase-4 regression risk; heterodyne's version is semantically cleaner. Homodyne should adopt the `diagnose_error` classification and immutable config copy pattern in Phase 4.

### NLSQ → CMC warm-start handoff

Homodyne performs warm-start setup inside `cmc/priors.py` as data-driven estimation functions (`estimate_contrast_offset_from_data`, `estimate_per_angle_scaling`) that operate on raw `(c2_data, t1, t2, phi_indices)` arrays and return scalar floats; boundary clamping is not present in `priors.py` — it happens upstream in `optimization_runner.py`. Heterodyne externalises the clamping logic into a dedicated `cmc/warmstart.py` (the module Phase 4 PR 3 will absorb into `priors.py`) providing two public functions: `clamp_params_to_interior(params: np.ndarray, parameter_names: list[str]) → (np.ndarray, list[str])` — the lower-level array path returning a fresh array and a list of clamped names — and `clamp_to_interior(result: NLSQResult, fixed_param_overrides: dict | None) → NLSQResult` — the higher-level path that returns a `dataclasses.replace(result, parameters=...)` copy so the original `NLSQResult` is never mutated. The absorber must preserve **both** these call signatures when merging into `priors.py`, because `cmc/core.py` calls `clamp_to_interior(NLSQResult)` and `cli/optimization_runner.py` calls `clamp_params_to_interior(np.ndarray, list[str])`. The geometric-margin path for log-space parameters (`log_space=True` in the registry, positive `min_bound`) computes `lo = min_bound * exp(margin * log(max/min))` and `hi = max_bound / exp(...)`, which is distinct from the linear path; both must survive the move. The `fixed_param_overrides` argument (which replaces stale NLSQ values for parameters that are currently fixed in the config) has no homodyne equivalent and must be retained as-is.

### cli/data_pipeline.py

The two pipelines share the same top-level gate — `validate_xpcs_data()` is called immediately after loading and raises `SystemExit(1)` on errors, logs warnings and continues — but diverge in the steps applied before and after. Homodyne's pipeline contains a `_exclude_t0_from_analysis()` step (line 35) that slices `t1[1:]`, `t2[1:]`, and `c2_exp[:, 1:, 1:]` before any angle filtering is applied; heterodyne has no equivalent step, because the heterodyne two-time model does not have the `D(t=0) → ∞` singularity that motivates homodyne's t=0 exclusion. Homodyne pools multi-angle data into a flat layout in `_pool_mcmc_data()` — producing `(n_phi * n_t * n_t,)` flat arrays for `mcmc_data`, `t1_pooled`, `t2_pooled`, `phi_pooled` — which is the shape contract the homodyne CMC model expects; heterodyne's `prepare_cmc_data()` (line 226) instead returns a shallow dict with `c2_data` as the full 3-D `(n_phi, n_t, n_t)` array, delegating any per-angle slicing to the CMC shard layer. Heterodyne also applies a mandatory diagonal correction (`_apply_diagonal_correction`) in `load_and_validate_data()` itself (lines 85–98), whereas homodyne defers diagonal handling to the calling layer. **Assessment:** the t=0 exclusion absence and the non-pooled shape are both correct heterodyne physics adaptations; the earlier diagonal correction placement in heterodyne is an improvement that reduces the risk of raw-diagonal data leaking into the physics engine and should be noted for homodyne Phase 4.
